...
func (userService *UserService) EditUser(req request.EditUser) error {
	...
	if user.RoleID == appTypes.Admin {
		return errors.New("不可以对管理员进行该操作")
	}
...
func (userService *UserService) ApprovingForVip(req request.ApprovingForVip, c *gin.Context) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		if req.IsPass {
			...
			err = tx.Save(&ask).Error
			if err != nil {
				return err
			}

			subject := "vip申请通过"
			body := `亲爱的用户[` + user.Email + `]，
<br/>
<br/>
你的vip申请已通过，欢迎你加入VIP的大家庭！！！<br/>
`
			_ = utils.Email(user.Email, subject, body)
			return nil
			...
			var user database.User
			err = tx.Where("uuid = ?", req.UUID).First(&user).Error
			if err != nil {
				return err
			}
			subject := "vip申请未通过"
...
<br/>
<br/>
很遗憾！你的vip申请经审核不符合加入会员的条件申请未通过，若对审核结果有异议你可以再次发起申请。<br/>
`
			_ = utils.Email(user.Email, subject, body)
